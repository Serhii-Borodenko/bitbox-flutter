...
class RawTransactions {
  ...
  static Future<List<String>?> sendRawTransaction(List<String?> hexes) async {
    final returnedTxIds = await RestApi.sendPostRequest("rawtransactions/sendRawTransaction", "hexes", hexes);

    if (returnedTxIds != null && returnedTxIds is List) {
      return List<String>.generate(returnedTxIds.length, (index) => returnedTxIds[index]);
    } else {
      return null;
    }
  }
